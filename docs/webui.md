...
- **编辑既有上游**（点卡片上的「编辑」）：表单是**同一份**，只是切换了模式 ——
  标题变成「✏️ 编辑上游：名称」、主按钮变成「保存修改」、出现「取消编辑」。
  保存时**替换原条目**而不是追加，并且不发生"先删后建"：
  删了再建会让那条上游的连接先断一次、它的工具先消失一次。
  - **改名是允许的**，但名称同时是工具名前缀，保存后 `{旧名}__{工具}` 会整批
    变成 `{新名}__{工具}` —— 提示里会写出这一点。
  - 校验失败（服务端 `config.Validate` 打回）时**保留表单内容**、不退出编辑
    模式；只有真正写盘成功才复位。
  - 编辑中那条被删掉时自动退出编辑模式 —— 否则提交会打到一个不存在的下标上，
    把改动静默丢掉。
